odoo.define('pos_sale_backport.screens', function (require) {
    ...
    const SaleOrderManagementScreen = screens.ScreenWidget.extend({
        ...
        renderElement: function () {
            ...
            this.$('.search-order').change(() => {
                const inputElement = document.getElementById("search-order-input");
                ...
                if (inputValue) {
                    ...
                    for (let i = 0; i < this.ordersToShow.length; i++) {
                        ...
                        if (textValue.toUpperCase().indexOf(inputValue.toUpperCase()) > -1) {
        ...
        fetch: async function () {
            try {
                let limit, offset;
                offset = this.nPerPage + (this.currentPage - 1 - 1) * this.nPerPage;
                limit = this.nPerPage;
        ...
        _fetch: async function (limit, offset) {
            const saleOrders = await this._getSaleOrderForCurrentPage(limit, offset);
        ...
        _getSaleOrderForCurrentPage: async function (limit, offset) {
            let domain = [['currency_id', '=', this.pos.currency.id]].concat(this.searchDomain || []);
            return await rpc.query({
                model: 'sale.order',
                method: 'search_read',
                args: [domain, ['name', 'partner_id', 'amount_total', 'date_order', 'state', 'user_id'], limit, offset],
            })
        },
        ...
        _onClickSaleOrder: async function (event) {
            ...
            if ($(event.target.parentNode).hasClass('click-sale-order')) {
                rowLineID = parseInt($(event.target.parentNode).data("item-index"));
            } else {
                rowLineID = parseInt($(event.target.parentNode.parentNode).data("item-index"));
            }
            if (rowLineID || rowLineID === 0) {
                ...
                await this.pos.gui.show_popup("selection", {
                    ...
                    list: [
                        {id: '0', label: _t('Apply a down payment'), item: false},
                ...
                await this.pos.gui.show_popup("error", {
                    ...
                    body: _t("There was an error selecting this order, please try again.")
                })
            }
        },
        ...
        _importSaleOrder: async function (selectedOrder, selectedOption) {
            ...
            let orderPartner = this.pos.db.get_partner_by_id(saleOrder.partner_id[0]);
            if (orderPartner) {
                ...
                try {
                    ...
                    this.pos.gui.show_popup('error', {
                        ...
                        body: _.str.sprintf(_t('There was a problem in loading the %s costumer.'), saleOrder.partner_id[1])
                    })
                }
                currentPOSOrder.set_client(this.pos.db.get_partner_by_id(saleOrder.partner_id[0]))
            }

            let orderFiscalPos = saleOrder.fiscal_position_id ? this.pos.fiscal_positions.find(
                (position) => position.id === saleOrder.fiscal_position_id[0]
            ) : false;
            if (orderFiscalPos) {
                currentPOSOrder.fiscal_position = orderFiscalPos;
            }
            let orderPricelist = saleOrder.pricelist_id ? this.pos.pricelists.find(
            ...
            if (selectedOption) {
                ...
                let product_to_add_in_pos = lines.filter(line => !this.pos.db.get_product_by_id(line.product_id[0])).map(line => line.product_id[0]);
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (newLine.get_product().tracking !== 'none' &&
                ...
                if (this.pos.config.down_payment_product_id) {
                    ...
                    let down_payment_product = this.pos.db.get_product_by_id(this.pos.config.down_payment_product_id[0]);
                    let down_payment_tax = this.pos.taxes_by_id[down_payment_product.taxes_id] || false;
                    let down_payment;
                    if (down_payment_tax) {
                        down_payment = down_payment_tax.price_include ? saleOrder.amount_total : saleOrder.amount_untaxed;
                    } else {
                        down_payment = saleOrder.amount_total;
                    }
                    let percentageValue;
                    await this.pos.gui.show_popup('number', {
                        title: _.str.sprintf(_t('Percetage of %s'), this.format_currency(saleOrder.amount_total)),
                        value: 0,
                        confirm: (number) => {
                    ...
                    if (percentageValue) {
                        down_payment = down_payment * parseFloat(percentageValue) / 100;
                    }
                    let newLine = new models.Orderline({}, {
                        pos: this.pos,
                        order: this.pos.get_order(),
                    ...
                    await this.pos.gui.show_popup('error', {
                        ...
                        body: _t('It seems that you didn\'t configure a down payment product in your point of sale.\
        ...
        load_new_partners: function () {
            ...
            return new Promise((resolve, reject) => {
                let fields = _.find(self.pos.models, (model) => {return model.label === 'load_partners';}).fields;
        ...
        _loadPartners: async function (partnerIds) {
            if (partnerIds.length > 0) {
                let fields = _.find(this.pos.models, function(model){ return model.label === 'load_partners'; }).fields;
        ...
        _addProducts: async function (ids, setAvailable=true) {
            ...
            let productModel = _.find(this.pos.models, (model) => model.model === 'product.product');
    ...
    gui.define_screen({name: 'sale_order', widget: SaleOrderManagementScreen});
        ...
        button_click: async function () {
            try {
                ...
                if (error.message === 'XmlHttpRequestError ') {
                    this.pos.gui.show_popup('error', {
                        title: _t("Network Error"),
                        body: _t("Cannot access order management screen if offline.")
                    })
                } else {
                    throw error;
                }
