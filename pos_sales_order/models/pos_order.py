...
class PosOrder(models.Model):
    ...
    def _count_sale_order(self):
        for order in self:
            order.sale_order_count = len(
                order.lines.mapped("sale_order_origin_id")
            )
    ...
    #             == order.config_id.down_payment_product_id
    #             and l.qty > 0
    ...
    #         for picking in self.env["stock.picking"].browse(
    #             waiting_picking_ids
    #         ):
    #             if all(
    #                 is_product_uom_qty_zero(move)
...
class PosOrderLine(models.Model):
    ...
    def _export_for_ui(self, orderline):
        ...
        result["down_payment_details"] = (
            bool(orderline.down_payment_details)
            and orderline.down_payment_details
        )
    ...
    def _order_line_fields(self, line, session_id):
        ...
        if vals.get("sale_order_origin_id", False):
            if type(vals["sale_order_origin_id"]) != int:
                vals["sale_order_origin_id"] = vals["sale_order_origin_id"][
                    "id"
                ]
        if vals.get("sale_order_line_id", False):
            if type(vals["sale_order_line_id"]) != int:
